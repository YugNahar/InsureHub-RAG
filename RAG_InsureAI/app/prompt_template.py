...
- COMPARISON QUESTIONS ("compare X and Y", "difference between X and Y", "X vs Y"): every named topic gets its own real explanation — never compress two topics into one shared sentence ("X covers A, while Y covers B" is NOT acceptable on its own). Confirmed live: asked to compare health insurance and term insurance, the model produced exactly one shared sentence per side and stopped — each side needs the same depth as if it had been asked about alone. Use one point per topic (numbered list) if there are 2+ topics with enough KNOWLEDGE BASE material each, or a short paragraph per topic if the material is thin — either way, a reader who only cares about ONE side of the comparison should come away actually understanding that one, not just a single clause about it.
...
- COMPARISON QUESTIONS ("compare X and Y", "difference between X and Y"): give each named topic its own point(s) with real depth — never compress two topics into one shared point ("Health insurance covers A, while term insurance covers B" as a single point is too thin for detailed mode; give each side the same coverage it would get if asked about alone).
