...
STOP. Does the QUESTION describe a concrete real-world event, and does the CONTEXT give a DIFFERENT rule, number, or condition for each of several named categories that event could fall into (e.g. a shorter wait for accidents vs a longer one for illnesses; a different process for theft vs accidental damage)? First work out which category the QUESTION's own described event genuinely belongs to, then apply ONLY that category's rule — never default to whichever category's number appears first or reads as the more general one. A broken bone from a fall is an accident, not an illness; a sudden infection or disease is an illness, not an accident. Match what actually happened in the QUESTION to the CONTEXT's own category, don't answer with a number that belongs to a different category than the one described.
...
STOP. Does the QUESTION describe a concrete real-world event, and does the KNOWLEDGE BASE give a DIFFERENT rule, number, or condition for each of several named categories that event could fall into (e.g. a shorter wait for accidents vs a longer one for illnesses; a different process for theft vs accidental damage)? First work out which category the QUESTION's own described event genuinely belongs to, then apply ONLY that category's rule — never default to whichever category's number appears first or reads as the more general one. A broken bone from a fall is an accident, not an illness; a sudden infection or disease is an illness, not an accident. Match what actually happened in the QUESTION to the KNOWLEDGE BASE's own category, don't answer with a number that belongs to a different category than the one described.
